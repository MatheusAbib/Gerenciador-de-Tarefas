...
export class TaskService {
  ...
  public getTasks(): Observable<Task[]> {
    return this._httpClient.get<Task[]>(`${this._apiUrl}/tasks`).pipe(
      tap(tasks => {
      const sortedTasks = this.getSortedTasks(tasks); //metodo que vai ordenar a lista baseado no nome
  ...
  public updateATasksInTheTasksList(updatedTask: Task): void {
    this.tasks.update(tasks => {
      // Filtra a tarefa que será removida (aquela que tem o mesmo id que updatedTask)
    ...
    //Deletar
      public deleteTask(taskId: string): Observable<Task> {
        return this._httpClient.delete<Task>(
          `${this._apiUrl}/tasks/${taskId}`)
